    ...
    return ( 
        ...
        <h2 className=" text-2xl font-bold text-sky-500">Popular news</h2>
            ...
            {popularNews.map((news) => {
                return <div key={news.title} className=" overflow-hidden py-5 px-4 bg-white rounded-md shadow-lg hover:shadow-sky-200 dark:hover:shadow-sky-950 duration-300  dark:bg-gray-900 flex flex-col gap-3 relative w-[18.5rem] max-sm:w-full group max-lg:w-96">
                    <Link href={"/"} className="">
                        <Image
                        src={news.image}
                        alt={news.name}
                        ...
                        <div>
                            ...
                            <h3 className=" text-xl font-semibold duration-300 text-gray-500  group-hover:text-sky-500">{news.title.split("").slice(0,30).join("")}...</h3>
                        </div>
