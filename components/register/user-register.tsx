...
const UserRegister: React.FC = () => {
  ...
  const socialAction = (action: string) => {
    ...
    .then((callback) => {
      if(callback?.error) {
        toast.error("something wet log to login 🥺")
      }
      if(callback?.ok && !callback?.error) {
        toast.success("welcome back 😄😄")
      }
    ...
    <div className=" mx-3">
      ...
      >
        ...
        <FamilyRegisterInput
          ...
          type="email"
          placeholder={
            variant === "Login" ? "Email or username" : "example@gmail.com"
          }
        />
        {variant === "Register" && (
          <div className=" mt-2">
            <div className=" w-full items-center gap-3 flex justify-start">
              <div className=" flex gap-2 items-center  ">
                ...
                <Input
                  type="radio"
                  name="gender"
                  className=" h-4 w-4 input-bordered "
                />
              </div>
              <div className=" flex gap-2 items-center  ">
                <Label htmlFor="gender" className="text-sm text-gray-500">
      ...
      <div className=" mt-4">
        ...
        <span className=" text-gray-500 px-2 absolute block text-center ml-56 -mt-3 text-sm bg-gray-200 dark:bg-gray-950 ">
          Or continue with
        </span>
        {/* providers option */}
        <div className=" flex w-full items-center justify-center  gap-3 mt-4 border-none ">
          <FamilyRegisterSocialButton icon={BsGoogle} onClick={() => socialAction("google")} />
          <FamilyRegisterSocialButton icon={BsGithub} onClick={() => socialAction("github")} />
        </div>
      </div>
    </div>
  );
